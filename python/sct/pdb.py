...
def pdb_res_line_parse(line, filename):
    ...
    if record in ['ATOM', 'HETATM']:
        ...
        if data['res_id'] in accept_resids:
            ...
            raise IOError(
                "Error: File %s contains unknown residue %s" %
                (filename, data['res_id']))
            data = {}
    else:
        data = {}
        ...
        for line in f:
            ...
            if len(data) != 0:
                ...
                if data['res_no'] != last_res_no:
                    # If a CHARMM residue name is used convert to standard
                    # naming
